defmodule AdventOfCode.Day02 do
  defmodule Submarine do
    ...
    @spec aim(t(), integer()) :: t()
    def aim(%__MODULE__{aim: aim} = submarine, delta) when is_integer(delta) do
      %__MODULE__{submarine | aim: aim + delta}
    end
    ...
    def calculate_destination_product(instructions) when is_list(instructions) do
      ...
      reducer = fn instruction, submarine ->
        with [instruction, number] <- String.split(instruction, " "),
             distance <- String.to_integer(number) do
          case instruction do
    ...
    def calculate_destination_product(instructions) when is_list(instructions) do
      ...
      reducer = fn instruction, submarine ->
        with [instruction, number] <- String.split(instruction, " "),
             distance <- String.to_integer(number) do
          case instruction do
            "down"    -> Submarine.aim(submarine, distance)
            "up"      -> Submarine.aim(submarine, -distance)
            "forward" -> Submarine.move(submarine, %{
              x: distance,
              y: submarine.aim * distance
            })
          end
        end
      end
